...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body style="background-color: White">
        ...
        <div id="tabs">
            ...
            <ul>
 <?php $commentout_tabs='yes';
          IF ($commentout_tabs=='yes')
          {  //   ALL OUT TO End BEFORE MONTIROING CONTENT line 562 ?>

<li><a href="#tabs-monitoring">Monitoring</a></li>

</ul>
<!--==============NOTE; 5/31/12 is quick fix for DTSC, show only site analysis, is copy of v5-->

 <?php     }   ELSE {  // End BEFORE MONTIROING CONTENT ?>
...
// end 8/31
  ...
  <?php } ?>   <!--     END First Comment out IF    -->
    ...
    <div class="section-title excavation">Summary  </div>

<!--  Summary Exclamation Mark/Check Mark -->
<?php



    // calculate values for Alerts
   // if(isset( $live_alert['OpenAlertCount_now_minus_x']['0']['count'])) {$underreview=$live_alert['OpenAlertCount_now_minus_x']['0']['count'];}
   // else{$underreview=0;}

  //  if(isset( $live_alert['OpenAlertCount_All_ReportDuration']['0']['count'])) {$totalalert=$live_alert['OpenAlertCount_All_ReportDuration']['0']['count'];}
  //  else{$totalalert=0;}

  //  (isset( $live_alert['OpenAlertCount_now_minus_x']['0']['count']))? ($underreview=$live_alert['OpenAlertCount_now_minus_x']['0']['count']):($underreview=0);
  //  (isset( $live_alert['OpenAlertCount_All_ReportDuration']['0']['count'])) ? ($totalalert=$live_alert['OpenAlertCount_All_ReportDuration']['0']['count']):($totalalert=0);


  //  $resolved=$totalalert-$underreview;
...
<table>
    <tr>
<td> <iframe src="https://feeder.terradex.com/lw_reporting/site_alert_openclosed_count_lastxdays/<?= $SiteID ?>/90" width="610" height="200" ></iframe></td>
    </tr>
</table>
...
<?php  ?>
                    ...
                    <table id="SiteDetails" cellspacing="0" border="0" style="border-collapse:collapse;">
                        <tr>
                            <td colspan="2">
                                <img src="https://feeder.terradex.com/lw_reporting/site_eventtype_pie_chart/20100101/<?   echo str_replace('-','',substr(date(DATE_W3C),0,10));  ?>/<?= $SiteID ?>/events" alt="Pie" style="width:500px;" />
                            </td>
                        </tr>
                    ...
                    <table id="SiteDetails" cellspacing="0" border="0" style="border-collapse:collapse;">
                        <tr>
                            <td colspan="2">
                                <img src="https://feeder.terradex.com/lw_reporting/site_eventtype_pie_chart/20110101/<?   echo str_replace('-','',substr(date(DATE_W3C),0,10));  ?>/<?= $SiteID ?>/alerts" alt="Pie" style="width:500px; " />
                            </td>
                        </tr>
   ...
   <div class="section-title excavation">Most Recent Alerts</div>
                ...
                <table class ="grid">
                    <tr>
                        <td colspan ="5" style="font-weight:bold"> <br/></td>
               ...
               // show different alert page for DTSC versus the others
                   if ($alert ['PayingParty'] == 'Department of Toxic Substances Control')
                            {
           ...
           // if recent alert, allow sharing BUT not for DTSC
                  if(($alert ['TransLocToLocID'] >'1643200') AND $alert ['PayingParty'] !== 'Department of Toxic Substances Control')
                          {


                      echo "<td><a href=\"http://feeder.terradex.com/dc_cleanupdeck/dc_share/". $alert ['TransLocToLocID'] ."\" target=\"_blank\" style=\"color:#0000FF\">Share</a></td>";
 ...
 <?php
          IF ($commentout_tabs=='yes')
    {  echo"";} ELSE { // skip all, goes to very end    ?>
            <!-- =========== BEGIN  Investigation  =============-->
...
<?php } ?><!--   end if user_id isset-->
                         <tr class="text_med">
                            <td width="33%">My Management Reports:</td>

                            <td>
                                <ul>
                                   <li><a class="text_med_bold" href="/report_max_control/co_report_mgt_reports/<?php echo $SiteID ?>/"style="color:#0000FF"  target="_blank">Click to View Management Reports</a></li>
                                </ul>
                            </td>
...
<?php if (isset($digcleanids['0'] ['tdx_appsid'] ))
{ ?>

           <tr class="text_med">
                            <td width="33%">My Advisories:</td>

                            <td>
                                <ul>
                                   <li><a class="text_med_bold" href="https://feeder.terradex.com/dc_cleanupdeck/lw_advisory_mgtpanel/<?php echo $digcleanids['0'] ['tdx_appsid'] ?>/"style="color:#0000FF"  target="_blank">Click to View Advisory Dashboard</a></li>
                                </ul>
                            </td>
